...
class ReceiptPDF(FPDF):
    def __init__(self, branch: str = "", maps_url: str = ""):
        super().__init__()
        self.branch = branch
        self.qr_bytes = None
    ...
    def header(self):
        ...
        self.cell(0, 5, _safe(f"Medical Test Invoice   |   {self.branch}"),
                  ln=True, align="C")
        self.ln(6)
...
def build_pdf(items, totals, name, phone, doctor, inv_date, branch="",
              maps_url="") -> bytes:
    pdf = ReceiptPDF(branch=branch, maps_url=maps_url)
    pdf.add_page()
